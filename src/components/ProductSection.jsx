...
export default function ProductSection() {
  ...
  const filteredProducts = products.filter(p => category === 'Semua' || p.kategori === category);
    ...
    <section id="produk" className="py-24 px-6 max-w-6xl mx-auto relative">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        <div>
          ...
          <h2 className="text-4xl md:text-5xl font-black text-[var(--color-coklat)] leading-tight">
            Produk <em className="italic text-[var(--color-coklat-muda)]">Pilihan</em> <span className="text-[var(--color-kuning-deep)]">Terbaik</span>
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {currentProducts.map(product => (
          <div key={product.id} className="bg-[var(--color-putih)] rounded-2xl overflow-hidden border-2 border-transparent hover:border-[var(--color-kuning)] shadow-sm hover:shadow-xl transition-all group flex flex-col h-full">
            <div className={`h-48 relative flex items-center justify-center p-6 ${product.bgColor}/20`}>
              {product.badge && (
              ...
              <img src={product.img} alt={product.nama} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" />
            </div>
            
            <div className="p-6 flex flex-col flex-1">
              <h3 className="font-bold text-lg text-[var(--color-coklat)] mb-2 line-clamp-2">{product.nama}</h3>
              <p className="text-sm text-[var(--color-coklat-mid)] line-clamp-2 mb-4 flex-1">{product.manfaat}</p>
              
              <div className="flex flex-wrap gap-2 mb-4">
                {product.ukuran.map(u => (
                  <span key={u.size} className="text-[10px] font-bold bg-[var(--color-krem)] text-[var(--color-coklat-muda)] px-2.5 py-1 rounded-md border border-[var(--color-krem-dark)]">
                    {u.size}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-2 mt-auto">
                <Link to={`/produk/${product.slug}`} className="flex-1 bg-transparent border-2 border-[var(--color-kuning)] text-[var(--color-coklat)] font-bold text-sm text-center py-2.5 rounded-lg hover:bg-[var(--color-kuning)] transition-colors">
                  <i className="fa-solid fa-eye mr-2"></i> Lihat
                </Link>
                <button onClick={() => setOrderProduct(product)} className="flex-1 bg-[var(--color-kuning)] text-[var(--color-coklat)] font-bold text-sm text-center py-2.5 rounded-lg hover:bg-[var(--color-kuning-deep)] transition-colors">
